...
At the beginning of the 1990s there were relatively few cross-border commuting inventors – most of them coming from Germany to Switzerland. However, until 2005 their number has increased for all neighboring countries to a combined total of over 1000 people. Not considering all these inventors and their corresponding patents could thus result in substantially underreporting Switzerland’s innovative output.

The extent of this bias can be seen in the next plot. Focus on the colored areas first. This time, they show the absolute number of patents invented by Swiss residents (orange) and cross-border commuters (purple) respectively. Patents have increased over time for both Swiss residents and cross-border commuters alike (see e.g. <a href = “xxx” target = “_blank”>xxx</a>  for a discussion about the general evolution of patenting in Switzerland). Also, we can immediately see that Switzerland’s patent output is clearly higher when also considering cross-border commuting inventors. The black solid line states the share of patents that were invented by cross-border commuters (i.e. the share of the purple area). It is quite substantial. Already in 1990, around 5% of all patents developed in Switzerland were inventions by cross-border commuters. This share doubled to around 10% in 2005 and has remained relatively stable ever since.
...
dat_fun <- reactive({
  ...
  # define and subset the data according to the selection
...
reshape_fun <- function(dataset){
  ...
  # use the same variable names and kick out tech_field No. 22 becuase of missings
  ...
  # calculate 3-year patent averages for regions/techfields
  plot_data <- plot_data %>% dplyr::arrange(p_year) %>% filter(as.numeric(p_year) <= 2015)
  ...
  # change type names and the data structure from wide to long format
  plot_data <- plot_data %>%
    rename(`Swiss residents` = Swiss_based, Commuters = commuters) %>%
    gather(key = Type, value = number, `Swiss residents`, Commuters)
...
# apply the function on both datasets
...
conditionalPanel(condition = "input.region_or_industry == 'CH_region'",
                 pickerInput(
                   ...
                   choices = as.list(as.character(unique(na.omit(plot_df$CH_region$regio_pat)))),
                   multiple = FALSE))
...
conditionalPanel(condition = "input.region_or_industry == 'CH_Techfields'",
                 pickerInput(
                   inputId = "OBS_ind",
                   label = "Choose a technology field", selected = "16",
                   choices = as.list(as.character(unique(na.omit(plot_df$CH_Techfields$tech_field)))),
                   multiple = FALSE))
...
Unsurprisingly, Swiss regions strongly differ with respect to cross-border commuting patents. The number of patents by cross-border commuters varies from only around 0.5-1% in Espace Mittelland to around 20% in Northwestern Switzerland and Ticino. The same is true for different technology fields.
