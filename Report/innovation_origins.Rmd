...
# plotly_labels <- paste(trunc(plot_data$number), plotly_labels)

# define 2nd axis:
scale_factor <- max(plot_data$number) / max(plot_data$share)

# ylim_absolute <- c(0, 6000)
# ylim_share <- c(0.5, 1)
...
# make the plot
p <- ggplot(plot_data)+
  geom_area(aes(x = p_year, y = number, fill = Type))+#, 
                # text = plotly_labels))+
  scale_fill_viridis(discrete = TRUE) +
  geom_line(aes(x = p_year, y = scale_factor * share), color = "black")+
  geom_point(aes(x = p_year, y = scale_factor * share), color = "black")+
  scale_y_continuous(#limits = c(100, 5500), 
                     name = "Number of Patents", 
                     sec.axis = sec_axis(~ ./scale_factor, name = "Share of cross-border commuters")) +
  # geom_line(aes(x = p_year, y = a + share*b), color = "black")+
  # geom_point(aes(x = p_year, y = a + share*b), color = "black")+
  # scale_y_continuous("Number of Patents", sec.axis = sec_axis(~ (. - a)/b, name = "Share of cross-border commuters")) +
  xlab("Year") +
...
# # load and prepare the data 
# plot_data <- df_list[["CH_overall"]]
# plot_data <- plot_data %>% filter(p_year <= 2015)
# 
# # make the plot
# ggplot(plot_data,
